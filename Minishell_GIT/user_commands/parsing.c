...
/*   Updated: 2024/07/11 20:52:03 by user             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    int op_len = ft_strlen(op);
    if (ft_strncmp(input + i, op, op_len) == 0 && !var->in_quotes)
    {
        if (i > var->start)
        {
            var->size += 1;
            var->args[var->size - 1] = ft_strndup(input + var->start, i - var->start);
        }
        var->size += 1;
        var->args[var->size - 1] = ft_strndup(op, op_len);
        var->start = i + op_len;
        return op_len;
    }
    return 0;
...
int pipes_and_redirects_handling(t_util *var, char *input, int i)
{
    ...
    if (operator_handling(var, input, i, "<<") > 0) return 2;
    if (operator_handling(var, input, i, "<<<") > 0) return 3;
...
{
	var->args = (char **)malloc((ft_strlen(var->input) + 1) * sizeof(char *));
	if (var->args == NULL)
		return (NULL);
	var->size = 0;
	var->in_quotes = 0;
	var->start = 0;
	var->quote_char = 0;
	for (int i = 0; i < ft_strlen(var->input); i++)
        var->args[i] = NULL;
	return (NULL);
}
...
char	**parse_user_input(t_util *var)
{
	...
	if (var->start < ft_strlen(var->input))
	{
		var->size += 1;
		var->args[var->size - 1] = ft_strndup(var->input
				+ var->start, ft_strlen(var->input) - var->start);
	}
	var->args[var->size] = NULL;
	return (var->args);
}
