...
/*   Updated: 2024/07/09 22:27:30 by edecorce         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_export_var(char *input, int from, int to)
{
	...
	formatted = remove_quotes(substr3);
	if (formatted == NULL)
	{
		ft_cmd_status(1);
		return (free(substr1), free(substr2), free(substr3), 0);
	}
	if (setenv(substr2, formatted, 1) != 0)
	{
		ft_cmd_status(1);
		perror("setenv failed\n");
	}
	ft_cmd_status(0);
	return (free(substr1), free(substr2), free(substr3), free(formatted), to + 2);
}
